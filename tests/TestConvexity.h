...
void TestConvexity::LinearFunctionConvexity(){
	...
	const ExprSymbol& x2a = Variable("xa");
	const ExprSymbol& x2b = Variable("xb");
	Function f2(x2a,x2b,x2a-x2b+1);
	TEST_ASSERT(conv.getConvexity(f2) == CONVEX_AND_CONCAVE);

	//multiplication by constants
	const ExprSymbol& x3a = Variable("xa");
	const ExprSymbol& x3b = Variable("xb");
	Function f3(x3a,x3b,x3a-2*x3b+1);
	TEST_ASSERT(conv.getConvexity(f3) == CONVEX_AND_CONCAVE);

	//multiplication by constants + distributivity
	const ExprSymbol& x4a = Variable("xa");
	const ExprSymbol& x4b = Variable("xb");
	Function f4(x4a,x4b,-4*(x4a-2*x4b+1));
